...
function ModalPost({ data, handleToggleModal, authorPost }) {
    return (
        <div className="fixed top-[25%] left-[50%] z-20">
            <div
                className="fixed top-0 left-0 right-0 bottom-0 bg-black bg-opacity-50 "
                onClick={handleToggleModal}
            ></div>
            <div className="absolute w-[70vw] h-[700px]  left-[50%] translate-x-[-50%] pb-[100px] bg-gray-100 rounded-[24px] overflow-hidden">
                <span className="absolute top-0 right-[20px] text-[3rem] cursor-pointer" onClick={handleToggleModal}>
                    &times;
                </span>
                <img className="w-full h-full object-cover " src={data.imagePost} alt="" />
